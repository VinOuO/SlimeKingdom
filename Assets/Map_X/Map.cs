...
public class Map : MonoBehaviour
{
    ...
    private void CreatLine(int row, int col, Vector3[] pos)
    {
        ...
        this.m_lines[row, col].AddComponent<Visible>();
    ...
    public Vector3 get_center()
    {
        ...
        if (Physics.Raycast(ray, out hit) && Visible.MeshisVisible)
        {
            return m_array[GetRowColByPos(hit.point).Key, GetRowColByPos(hit.point).Value];
        }
        else
        {
    ...
    void Update()
    {
        ...
        else if (!Visible.MeshisVisible)
        {
            this.CreatMesh(new Vector3(0,-10000,0));
        }
    }
    ...
    private void DrawMesh(Vector3 pos)
    {
        try
        {
            ...
            else if (mesh == null || CacheMesh == null)
            {
                ...
                if (mesh == null)
                {
                    Debug.LogError("該區域不能選擇！");
                    return;
                }

                if (mesh.vertices.Length > 300)
                    Debug.LogErrorFormat("頂點數據過大 {0} :{1}", rw.Key, rw.Value);

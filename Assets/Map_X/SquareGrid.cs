...
public class SquareGrid : BaseGrid
{
    ...
    {
        KeyValuePair<int, int> info = Map.mIns.GetRowColByPos(pos);

        if ((info.Key + this._coefficient - 1 >= Map.mIns.ArrRow) || (info.Value + this._coefficient - 1 >= Map.mIns.ArrCol))
        {
            this._vertexes = null;
            return;
        }
        ...
        for (int i = 0; i < this._coefficient; ++i)
        {
            ...
            {
                this._vertexes[index++] = Map.mIns.Array[info.Key + i, info.Value + j] + new Vector3(0, 0.1f, 0);
            }
        }
    }
